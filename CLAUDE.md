...
One-page site for Diogo Almeida's private maths tutoring (explicações de Matemática, 7º–12º ano, na Maia ou online). Astro 5 static site with Tailwind CSS v4. Live at https://diogoexplicacoes.web.app (Firebase Hosting, project `diogoexplicacoes`); source at https://github.com/diogofca/SiteExplicacoes. No backend, no tests.

All user-facing copy is **Portuguese (pt-PT)** and deliberately minimal — the owner wants very little text, no self-promotion beyond one discreet line of grades, no exam-prep marketing. Lessons are "na Maia ou online" — never say "presencial". Keep it that way when editing.
...
- **`src/data/site.ts`** is the single source of content: name, e-mail (`diogoalmeidaexplicacoes@outlook.com`), city/modality (`cidade`, `local`), grades (`faculdade`, `mediaSecundario`, `mediaFaculdade` — the one discreet line in `Hero.astro`), levels/prices (13 €/h 3º ciclo, 15 €/h secundário), steps and nav links. Change copy or prices here, not in components.
